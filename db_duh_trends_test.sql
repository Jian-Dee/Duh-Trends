...
CREATE DEFINER=`root`@`localhost` PROCEDURE `AddItemToStock`(IN `renter_id_param` INT, IN `item_id_param` VARCHAR(10), IN `quantity_param` INT, IN `item_price_param` FLOAT(10,2), IN `item_description_param` TEXT)
BEGIN
    DECLARE item_exists INT;
    ...
    IF item_exists > 0 THEN
        ...
        INSERT INTO tbl_item_stock (item_id, quantity, item_price, item_description, item_type_id)
        SELECT item_id_param, quantity_param, item_price_param, item_description_param, item_type_id
        FROM tbl_items
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `AddNewSale`(IN `staff_id_param` INT, IN `item_id_param` VARCHAR(10), IN `quantity_param` INT)
BEGIN
    ...
    IF v_renter_id IS NULL THEN
        ...
        SET MESSAGE_TEXT = 'Item does not have a renter';
    END IF;

    IF stock_quantity >= quantity_param THEN
        START TRANSACTION;

        INSERT INTO tbl_sales (user_id) VALUES (staff_id_param);
        ...
        VALUES (item_id_param, quantity_param, v_renter_id);
